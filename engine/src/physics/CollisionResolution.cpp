...
void CollisionResolution::resolveCollision(const Collision& collision)
{
    ...
    if (typeA == BodyType::DynamicBody)
    {
        ...
        if (typeB == BodyType::DynamicBody)
        {
            ...
            resolveAdvancedDynamicCollision(dynamicBodyA, dynamicBodyB, collision.normal, collision.penDepth, collision.contactPoints, collision.contactCount);
        }
        ...
        else if (typeB == BodyType::StaticBody)
        {
            ...
            resolveAdvancedDynamicStaticCollision(dynamicBodyA, staticBodyB, collision.normal, collision.penDepth, collision.contactPoints, collision.contactCount);
        }
    }
    ...
    else if (typeA == BodyType::StaticBody)
    {
        ...
        if (typeB == BodyType::DynamicBody)
        {
            ...
            resolveAdvancedDynamicStaticCollision(dynamicBodyB, staticBodyA, -collision.normal, collision.penDepth, collision.contactPoints, collision.contactCount);
        }
    }
}

//Resolve a collision between a dynamic body and a static body
void CollisionResolution::resolveBasicDynamicStaticCollision(DynamicBody* dynamicBody, StaticBody* staticBody, const Vector2& normal, float penDepth)
{
...
void CollisionResolution::resolveBasicDynamicCollision(DynamicBody* bodyA, DynamicBody* bodyB, const Vector2& normal, float penDepth)
{
...
void CollisionResolution::resolveAdvancedDynamicStaticCollision(DynamicBody* dynamicBody, StaticBody* staticBody, const Vector2& normal, float penDepth, const std::vector<Vector2>& contactPoints, int contactCount)
{
    ...
    for (int i = 0; i < contactCount; i++)
    {
        ...
        if (impulse.getLength() == 0.0f) continue;
...
void CollisionResolution::resolveAdvancedDynamicCollision(DynamicBody* bodyA, DynamicBody* bodyB, const Vector2& normal, float penDepth, const std::vector<Vector2>& contactPoints, int contactCount)
{
    //Get properties
